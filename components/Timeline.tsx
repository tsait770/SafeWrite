    ...
    <div className="relative w-full h-full bg-[#0F0F10] shadow-2xl flex flex-col overflow-hidden text-slate-200">
      ...
      <div className="px-8 pb-6 border-b border-white/5 shrink-0 flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-0.5">TIME MACHINE MANAGEMENT</p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto px-8 py-10 no-scrollbar relative">
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white">LIVE EDITION ACTIVE</span>
          </div>
          <button
            onClick={onCreateMilestone}
            className="px-5 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-500 text-[10px] font-black uppercase tracking-[0.2em] active:scale-95 transition-all shadow-lg"
          >
            標記里程碑
          </button>
        </div>
        ...
        <div className="space-y-12 relative">
          {history.length === 0 ? (
            <div className="pl-12 pt-4">
              <p className="text-[10px] text-slate-600 leading-relaxed uppercase tracking-[0.2em] italic font-black">
                系統正在監控變動...<br/>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {snapshot.type === SnapshotType.MILESTONE && (
                      <span className="text-[10px] font-black bg-amber-500 text-black px-2 py-0.5 rounded tracking-[0.2em]">
                        MILESTONE
                      </span>
                    )}
                  </div>
                  ...
                  <p 
                    className="text-[1.1rem] text-slate-300 line-clamp-2 leading-[2.2] mb-4 cursor-pointer hover:text-white transition-colors font-serif-editor" 
                  ...
                  <div className="flex items-center space-x-6 opacity-0 group-hover:opacity-100 transition-all">
                    <button 
                      ...
                      className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 hover:text-blue-400 flex items-center space-x-2"
                    >
                    ...
                    <button onClick={() => onPreview(snapshot)} className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-slate-300">
                      PREVIEW
                    </button>
                  </div>
                </div>
      ...
      <footer className="p-8 bg-black/40 border-t border-white/5 space-y-8 shrink-0">
        <div className="flex items-start space-x-4">
          ...
          <p className="text-[10px] text-slate-500 leading-relaxed uppercase tracking-[0.2em] font-black">
        ...
        <div className="flex items-center justify-between px-1">
          ...
          <button 
            ...
            disabled={isClearing}
            className="text-[10px] font-black text-red-500/60 hover:text-red-500 uppercase tracking-[0.2em] transition-all flex items-center space-x-3"
          >
            {isClearing ? (
