    ...
    <div className="fixed inset-0 z-[1000] bg-black flex flex-col animate-in slide-in-from-right duration-500 overflow-hidden">
      <header className="h-24 px-8 pt-[env(safe-area-inset-top,0px)] flex items-center justify-between border-b border-white/5 bg-black/80 backdrop-blur-3xl shrink-0">
        <div className="flex items-center space-x-5">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#8E8E93] font-black uppercase tracking-[0.2em] mt-0.5">SECURITY & PROTECTION</p>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-2xl mx-auto space-y-12">
          ...
          <section className="bg-gradient-to-br from-[#1C1C1E] to-[#121214] rounded-[44px] p-10 border border-white/5 shadow-2xl space-y-6">
            <div className="flex items-center space-x-5 mb-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#D4FF5F] font-black uppercase tracking-[0.2em]">PROTECTION PROTOCOL</p>
              </div>
            </div>
            <p className="text-[10px] text-gray-300 leading-relaxed font-black uppercase tracking-[0.2em]">
              自動快照能確保您的文字在意外中獲得救贖。快照保留期限設定後，系統將根據您的選擇自動清理過期快照以節省空間，但里程碑版本將永遠保留。
            </p>
          </section>
          ...
          {settings.autoSnapshotEnabled && (
            <div className="space-y-12 animate-in fade-in slide-in-from-top-4 duration-700">
              ...
              <section className="space-y-6">
                ...
                <div className="bg-[#1C1C1E] rounded-[44px] p-8 border border-white/5">
                   ...
                   <div className="mt-8 p-6 bg-amber-500/5 rounded-3xl border border-amber-500/10">
                      <p className="text-[10px] text-amber-500/80 leading-relaxed font-black uppercase tracking-[0.2em]">
                        <i className="fa-solid fa-circle-exclamation mr-2"></i>
                        僅自動快照受清理期限影響，里程碑版本將獲得終身永久保留保障。
                      </p>
                   </div>
                </div>
              </section>
              ...
              <section className="space-y-6">
                <h3 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] px-2 flex items-center">
                ...
                <div className="bg-[#1C1C1E] rounded-[44px] p-8 space-y-10 border border-white/5">
                  <div className="flex bg-black/40 rounded-[28px] p-1.5 h-16 border border-white/5">
                    <button 
                      ...
                      className={`flex-1 rounded-[22px] text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 ${settings.autoSnapshotMode === 'interval' ? 'bg-blue-600 text-white shadow-xl scale-[1.02]' : 'text-gray-500 hover:text-gray-300'}`}
                    >
                      固定時間制
                    </button>
                    <button 
                      onClick={() => onUpdate({ ...settings, autoSnapshotMode: 'idle' })}
                      className={`flex-1 rounded-[22px] text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 ${settings.autoSnapshotMode === 'idle' ? 'bg-blue-600 text-white shadow-xl scale-[1.02]' : 'text-gray-500 hover:text-gray-300'}`}
                    >
                      動作停止制
                    </button>
                  </div>
                  ...
                  <div className="space-y-8">
                    <div className="flex justify-between items-end px-1">
                      <div>
                        ...
                        <p className="text-[10px] text-gray-600 font-black uppercase tracking-[0.2em] mt-0.5">Adjustment Range</p>
                      </div>
          ...
          <section className="bg-gradient-to-tr from-[#1E293B] to-[#0F172A] p-10 rounded-[44px] border border-blue-600/10 flex flex-col items-center space-y-8 shadow-2xl relative overflow-hidden">
             ...
             <div className="space-y-4 text-center">
                <h4 className="text-[10px] font-black text-white uppercase tracking-[0.2em]">SAFEWRITE 數據救贖保障</h4>
                <p className="text-[10px] text-gray-400 leading-relaxed font-black uppercase tracking-[0.2em]">
                  採用本地先行與即時快照技術。即使發生斷電或程式閃退，系統仍能精準救贖每一道創作思緒。
                </p>
             </div>
          </section>
        </div>
      ...
      <footer className="absolute bottom-0 inset-x-0 p-8 sm:p-12 bg-gradient-to-t from-black via-black/90 to-transparent shrink-0">
         <div className="max-w-2xl mx-auto">
            <button 
              onClick={onClose}
              className="w-full py-6 bg-white text-black font-black text-[10px] uppercase tracking-[0.2em] rounded-[44px] shadow-2xl active:scale-[0.98] transition-all duration-300"
            >
               確 認 設 定
            </button>
         </div>
